...
class PromptEncoder(nn.Module):
    def __init__(
        ...
        self.mask3D_downscaling = nn.Sequential(
    ...
    def _embed_masks(self, masks: torch.Tensor) -> torch.Tensor:
        """Embeds mask inputs."""
        mask_embedding = self.mask3D_downscaling(masks)
    ...
    def forward(
        ...
        bs = self._get_batch_size(text_embeddings, masks)
        ...
        sparse_embeddings = torch.empty((bs, 0, self.embed_dim), device=text_embeddings.device, dtype=dense_embeddings.dtype)

        if text_embeddings is not None:
            sparse_embeddings = torch.cat([sparse_embeddings, text_embeddings.unsqueeze(dim=1)], dim=1)
...
class PositionEmbeddingRandom(nn.Module):
    ...
    def forward(self, size: Tuple[int, int, int]) -> torch.Tensor:
        ...
        grid = torch.ones((h, w, d), device=device, dtype=self.positional_encoding_gaussian_matrix.dtype)
        # print("++ self.positional_encoding_gaussian_matrix.dtype", self.positional_encoding_gaussian_matrix.dtype)
