...
def sliding_window_inference(
    ...
    **kwargs: Any,
    ...
    # if len(inputs)>1:
    #     inputs = inputs[0]
    #     promptargets = inputs[1]
    # else:
    #     raise ValueError(f"expected ingput: images and promptargets, but Input length: {len(inputs)}")
    ...
    for slice_g in range(0, total_slices, sw_batch_size):
        ...
        print("++swindow slice_g",inputs.shape)
        print("++ window_data.shape",window_data.shape)
        seg_prob = predictor(images=window_data, *args, **kwargs) # batched patch segmentation
        seg_prob = seg_prob.get('logits')
        # if seg_prob.shape[1] != window_data.shape[1]:
        #     seg_prob = torch.zeros(window_data.shape).to(sw_device)
