...
from LaMed.src.dataset.multi_dataset import MultiSegDataset, TextDatasets, UniDatasets
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        print('++ Only save projector and embed_tokens in pretrain')
...
def main():
    ...
    if training_args.lora_enable:
        ...
        for n, p in model.named_parameters():
            ...
            if any(
                    [x in n for x in ['mm_projector', 'embed_tokens', 'lm_head', 'seg_projector']]#step 3
            ):
                p.requires_grad = True

            if any(
                    [x in n for x in ['vision_module']]#step 4
            ):
    ...
    else:#tune_mm_mlp_adapter
        for n, p in model.named_parameters():
            p.requires_grad = False
            if any(
                    [x in n for x in ['mm_projector', 'embed_tokens', 'lm_head']]#step 2
            ):
                p.requires_grad = True
